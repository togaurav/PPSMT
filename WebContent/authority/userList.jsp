		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
		   <tr>
		    <s:if test="%{#request.pginfo.pageNo > 1 }">
		     <a href="${pageContext.request.contextPath}/default/userList.action?currentPage=1" target="main-iframe">首页</a>
		    </s:if>
		    <s:if test="%{#request.pginfo.pageNo >= 3 }">
		     <a href="${pageContext.request.contextPath}/default/userList.action?currentPage=<s:property value="#request.pginfo.pageNo - 1" />" target="main-iframe">前一页</a>
		    </s:if>
		
		    	第<s:property value="#request.pginfo.pageNo" />/<s:property value="#request.pginfo.sumPage" />页
		    
		    <s:if test="%{#request.pginfo.pageNo <= #request.pginfo.sumPage - 2 }">
		     <a href="${pageContext.request.contextPath}/default/userList.action?currentPage=<s:property value="#request.pginfo.pageNo + 1" />" target="main-iframe">下一页</a>
		    </s:if>
		    <s:if test="%{#request.pginfo.pageNo <  #request.pginfo.sumPage}">
		     <a href="${pageContext.request.contextPath}/default/userList.action?currentPage=<s:property value="#request.pginfo.sumPage" />" target="main-iframe">末页</a>
		    </s:if> 
		   </tr>
		  </table>
