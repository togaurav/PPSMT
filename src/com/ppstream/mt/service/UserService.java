...
public class UserService {
	...
	public User getUserByNameAndPwd(String username,String password){
		String hql = "from User where userName = ? and password = ? and status = '1'";  // 在启用账户中查找   
	...
	public void addOrUpdateUser(Integer userId,String userName,String password,String email,String roleIds,Integer groupLeader,Integer groupId,String nickName,String subPhone){
		User user = null;
		if(userId == null){
			user = new User();
		}else{
		...
		for(int i=0;i<ids.length;i++){
			Role role = baseDao.get(Role.class, ids[i]);
...
//		user.setRegIp(ip);
	...
	public HashMap<String, HashMap<String, HashSet<GPrivilege>>> setPrivilegeToMap(HashMap<String, HashMap<String, HashSet<GPrivilege>>> maps,Set<Privilege> privileges){
		...
		while (privilegeIterators.hasNext()) {
			...
			if (maps.containsKey(typeName)) {
				...
				if (cateMap.containsKey(cateName)) { // 新增小类
					HashSet<GPrivilege> privilegeSet = cateMap
							.get(cateName);
					privilegeSet.add(gp);
				} else { // 新增中类
